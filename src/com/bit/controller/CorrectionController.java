		...
		if(path.equals("/correction/list.do")) {
			...
			try {
				if(req.getParameter("page")!=null) {
					...
					List<CorrectionDTO> list = dao.selectAll(page, select, input);
					out.print("{\"correction\":");
					out.print(list);
					out.print(",\"count\":"+ dao.count(select, input));
					out.print("}");
					return;
				} else {
					List<CorrectionDTO> list = dao.selectAll();
					req.setAttribute("count", dao.count("title",""));
